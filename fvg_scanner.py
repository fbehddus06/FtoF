"""
FVG 컨플루언스 스캐너 v4 (자동 폴백)
====================================
거래소가 미국 IP를 차단할 경우 자동으로 다른 거래소 시도.
...
EXCHANGE_PREFERRED = os.environ.get('EXCHANGE', '').lower()

KEY_FIBS = (0.382, 0.5, 0.618, 0.786)
ENTRY_MAX_PROXIMITY = 0.05
...
def analyze(symbol, c4h, c1d, c1w):
    ...
    for k, v in fib.items():
        ...
        if best is None or total > best['score']:
            best = {
                ...
                'risk': risk, 'reward': reward, 'score': total,
                'score_breakdown': {
                    'entry': score_entry, 'target': score_target,
                    'weekly': score_weekly, 'daily': score_daily,
                    'lvn': score_lvn, 'rr': score_rr,
                },
            }
    ...
    return {
        'symbol': symbol, 'direction': direction, 'current': current,
        'entry_level': entry_level, 'entry_price': entry_price,
        'entry_prox': entry_prox,
        'target_level': best['level'], 'target_price': best['price'],
        'stop_price': best['stop_price'], 'rr': best['rr'],
        'risk_pct': abs(best['risk'] / current) * 100,
        'reward_pct': abs(best['reward'] / current) * 100,
        'in_daily': best['in_daily'], 'in_weekly': best['in_weekly'],
        'in_lvn': best['in_lvn'], 'score': best['score'],
        'breakdown': best['score_breakdown'],
        'swing_high': swing_high, 'swing_low': swing_low, 'fib': fib,
        'candles_4h': c4h, 'daily_fvgs': daily_fvgs,
        'weekly_fvgs': weekly_fvgs, 'lvns': lvns,
    }
...
def render_chart(r):
    ...
    for k, v in r['fib'].items():
        ax.axhline(v, color='#d4a64a', alpha=0.7, linestyle='--', linewidth=0.6)
        ax.text(0.5, v, f' {k}', color='#d4a64a', fontsize=8,
                va='bottom', alpha=0.85, fontweight='bold')
...
def format_caption(r):
    ...
    b = r['breakdown']
    if r['score'] >= 85: grade = '⭐⭐⭐ 완벽'
    elif r['score'] >= 70: grade = '⭐⭐ 우수'
    elif r['score'] >= 55: grade = '⭐ 양호'
    else: grade = '△ 부분'
...
# ─── 거래소 자동 폴백 ────────────────────────────────
# 미국 차단 적고 알트코인 풍부한 거래소 우선
EXCHANGE_CONFIGS = [
    ('gate',          {'options': {'defaultType': 'swap'}},   '/USDT:USDT'),
    ('mexc',          {'options': {'defaultType': 'swap'}},   '/USDT:USDT'),
    ('bitget',        {'options': {'defaultType': 'swap'}},   '/USDT:USDT'),
    ('kucoinfutures', {},                                      '/USDT:USDT'),
    ('htx',           {'options': {'defaultType': 'swap'}},   '/USDT:USDT'),
    ('okx',           {'options': {'defaultType': 'swap'}},   '/USDT:USDT'),
    ('bybit',         {'options': {'defaultType': 'linear'}}, '/USDT:USDT'),
    ('binance',       {'options': {'defaultType': 'future'}}, '/USDT:USDT'),
]


async def find_working_exchange():
    configs = list(EXCHANGE_CONFIGS)
    if EXCHANGE_PREFERRED:
        configs.sort(key=lambda c: 0 if c[0] == EXCHANGE_PREFERRED else 1)

    for name, opts, suffix in configs:
        exchange = None
        try:
            exchange = getattr(ccxt, name)(opts)
            tickers = await exchange.fetch_tickers()
            valid = [(s, t) for s, t in tickers.items()
                     if s.endswith(suffix) and
                     (t.get('quoteVolume') or t.get('baseVolume'))]
            if len(valid) >= 10:
                print(f"  ✓ {name.upper()} 연결 성공 · 종목 {len(valid)}개")
                return exchange, name, suffix, valid
            print(f"  ✗ {name.upper()}: 종목 {len(valid)}개 (부족)")
...
# ─── 메인 ────────────────────────────────────────────
async def scan():
    print(f"[{datetime.utcnow():%Y-%m-%d %H:%M}] 스캔 시작")
    print(f"  상위 {TOP_N}개 · 최소 점수 {MIN_SCORE}/100")
    print(f"  거래소 자동 탐색 중...")

    exchange, name, suffix, valid_pairs = await find_working_exchange()

    try:
        valid_pairs.sort(key=lambda x: float(
            x[1].get('quoteVolume') or x[1].get('baseVolume') or 0), reverse=True)
        top = valid_pairs[:TOP_N]
        ...
        header = (
            f"*🔍 FVG 컨플루언스 스캐너*\n"
            f"{datetime.utcnow():%Y-%m-%d %H:%M} UTC · {name.upper()}\n"
